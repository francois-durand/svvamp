    ...
    * :meth:`is_um_`:
    ...
    def _vote_strategically(self, matrix_duels_s, strength, c, n_m):
        ...
        candidates_not_c = np.concatenate(
            (np.array(range(c), dtype=int), np.array(range(c + 1, self.profile_.n_c), dtype=int))
        )
        ...
        for i in range(self.profile_.n_c - 1):
            ...
            possible_y = np.logical_and(
                np.logical_and(array_x, matrix_u[:, c] == the_d), np.any(w_sup[array_f, :] >= the_d, 0)
            )
        ...
        matrix_duels_m = (
            n_m
            * np.triu(np.ones((self.profile_.n_c, self.profile_.n_c)), 1)[reciprocal_lambda, :][:, reciprocal_lambda]
        )
    ...
    def _cm_main_work_c_(self, c, optimize_bounds):
        ...
        if success_tb:
            self._update_sufficient(
                self._sufficient_coalition_size_cm, c, n_m - 1, "CM: Update sufficient_coalition_size_cm = n_m - 1 ="
            )
        ...
        if success_cowinner:
            self._update_sufficient(
                self._sufficient_coalition_size_cm, c, n_m, "CM: Update sufficient_coalition_size_cm = n_m ="
            )
            ...
            self._update_necessary(
                self._necessary_coalition_size_cm, c, n_m, "CM: Update necessary_coalition_size_cm = n_m ="
            )
