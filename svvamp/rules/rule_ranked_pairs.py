...
class RuleRankedPairs(Rule):
    ...
    * :meth:`is_um_`: Non-polynomial or non-exact algorithms from superclass :class:`Rule`.
    ...
    def candidates_by_scores_best_to_worst_(self):
        """1d array of integers. ``candidates_by_scores_best_to_worst[k]`` is the ``k``-th candidate by
    ...
    def scores_best_to_worst_(self):
        ...
        ``scores_best_to_worst[k, j]`` is the score of the ``k``-th best candidate of the election against
        the ``j``-`th`. It is the result in :attr:`~svvamp.Population.matrix_duels_rk` if this duels was
