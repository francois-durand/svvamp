...
class PopulationVMFHypersphere(Population):
    ...
    def __init__(self, V, C, vmf_concentration, vmf_probability=None,
        ...
        Then, v's utility vector is sent onto the spheroid that is the image
        of the sphere by a dilatation of factor 'stretching' along the
        direction (1, ..., 1). E.g., if stretching = 1, we stay on the unit
        sphere of R^C. Cf working paper Durand et al. 'Geometry on the Utility
        Sphere'.

        N.B.: if stretching != 1, it amounts to move the poles. For example, if
        the pole [1, 0, 0, 0] is given and stretching = 0, then the actual pole
        will be [0.75, - 0.25, - 0.25, - 0.25] (up to a multiplicative
        constant).

        poles are normalized before being used. So, the only source of
        concentration for group i is vmf_concentration[i], not the norm of
        vmf_pole[i]. If pole is None, then each pole is drawn independently
        and uniformly on the sphere.
