...
class IRVAverageResult(ElectionResult):
    """Results of an election using IRV-Average (Nanson-like)
    ...
    the same Plurality score, the candidate with lowest index is declared the
    winner.
    ...
    def __init__(self, population, **kwargs):
        ...
        self._log_identity = "IRV_AVERAGE_RESULT"
    ...
    def _count_ballots(self):
        ...
        score_temp = np.copy(self.pop.plurality_scores_rk).astype(float)
    ...
    @property
    def scores(self):
        """2d array of integers. ``scores[r, c]`` is the Plurality score of
    ...
    def candidates_by_scores_best_to_worst(self):
        ...
        the same round, they are sorted by Plurality score at that round (less
        votes are eliminated first) and, in case of a tie, by their index
        (highest indexes are eliminated first).
        
        By definition / convention,
        ``candidates_by_scores_best_to_worst[0]`` = :attr:`~svvamp.IRVAverage.w`.
        """
