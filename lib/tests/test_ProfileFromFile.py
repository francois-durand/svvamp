...
def test_from_cvr():
    file = os.path.join(os.path.dirname(__file__), 'example_ballots.cvr.csv')
    profile = ProfileFromFile(file, sort_voters=False)
    assert profile.preferences_ut.shape == (15, 6)
    assert len(profile.labels_candidates) == 6
...
def test_sort_candidates():
    file = os.path.join(os.path.dirname(__file__), 'example_ballots.csv')
    profile = ProfileFromFile(file, sort_voters=False, sort_candidates=True)
    assert profile.preferences_ut.shape == (86, 11)
    assert list(profile.labels_candidates) == [
        'Frank', 'Hector', 'Grace', 'Alice', 'David', 'Jack', 'Isabel', 'Karin', 'Catherine', 'Bob', 'Ellen'
    ]
